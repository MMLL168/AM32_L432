...
### 修改原因
17k RPM 穩定運行約 10 秒後突然失步，bemf_timeout=0。確認為**機率性假 ZC**：17k RPM = 11,900 次換相/秒，每次換相有極低機率 PWM 振鈴穿透 filter_level=20，累積 ~119,000 次後統計必中一次。根本原因：COMP1 輸入無 RC 濾波，PWM 切換振鈴（持續 2-3μs）直接到達比較器。

嘗試：interruptRoutine 每次確認間加 150ns NOP 延遲（展開窗口至 2.85μs）→ 換相時序偏移，反而更糟，已還原。

### 解決方式
**`Inc/targets.h`**：NOMINAL_PWM 48kHz → 24kHz

24kHz 下 OFF 窗口從 3.7μs → 7.5μs（82% duty）；振鈴（~2μs）沉澱後剩 5.5μs 乾淨 BEMF。假 ZC 機率降低，失步間隔延長。

**最終確認：16-17k RPM 穩定運行，偶爾（~10秒）機率性失步為此硬體的底限。**
硬體根本解法：COMP1 輸入加 4.7nF + 10kΩ RC 濾波（fc≈3.4kHz），軟體端已達極限。
